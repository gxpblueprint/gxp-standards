...
List records without downloading every content body:
...
Retrieve the first published Part 11 URS requirement:

```bash
curl https://www.gxpblueprint.com/api/v1/records/019f4cbc-ba4b-73ca-bef1-e538397cd123.json
```
...
const requirements = catalog.records.filter((record) => record.type === 'urs_requirement');
```
